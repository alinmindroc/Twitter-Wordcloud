...
occurence count for every word in the stream or for a number of words specified by the user. It can be used as the backend for a
...
and set them in [wordcloud.py](tagcloud_files/wordcloud.py) at lines 12-16 

#### How to run the app inside the docker container:
(this will download the docker images for redis and python:2.7.9)  

1. Install docker and docker-compose
2. To run the app with the default args: `sudo docker-compose up`
...
5. Replace the value of the redis host at line 20 in [wordcloud.py](tagcloud_files/wordcloud.py) with the IP of the redis server. You can use 'localhost' if the server is local.
